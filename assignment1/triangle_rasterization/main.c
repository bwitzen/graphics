 ...
 * Feb 8, 2013
 *
 * File slightly adjusted by us. See original description below.
 *
 * ************************************************************************** */
...
void PutPixel(int x, int y, byte r, byte g, byte b)
{
    ...
    // RGBRGBRGB...
    // adjusted to include the color_by_putpixel_count method
    if (color_by_putpixel_count) {
        if (framebuffer[3*(framebuffer_width*y+x)] != 254)
            framebuffer[3*(framebuffer_width*y+x)] += 127;
